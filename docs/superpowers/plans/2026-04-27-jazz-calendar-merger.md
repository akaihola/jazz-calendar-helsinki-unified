...
6. Bootstrap (Chunk 10) is the only chunk that mutates anything outside the repo (creates the GitHub repo, enables Pages). Skip it if `gh repo view akaihola/jazz-calendar-helsinki-unified` already returns success.
...
- [ ] Extract the Manala VEVENTs from the captured fixtures (the spec's confirmed collision example: 2026-04-30 17:00 EEST at Ravintola Manala, [spec §5.2]).
- [ ] Hand-craft the recurring-event and missing-DTSTART fixtures by hand (≤15 lines each).
...
Use a fixed `now = datetime(2026, 4, 27, 6, 0, tzinfo=UTC)`.
...
- [ ] `test_recurring_open_ended_kept_even_if_master_old` — load `recurring_open_ended.ics` (master DTSTART 2014, no UNTIL) ⇒ True.
- [ ] `test_recurring_with_expired_until_dropped` — load `recurring_with_until.ics` (UNTIL 2014-06-01) ⇒ False.
...
The merge code reads two URLs (`SUOMIJAZZ_URL`, `GCAL_URL`) and reads the previous file from `docs/calendar.ics`. Tests use the test seam — env vars `SUOMIJAZZ_URL` and `GCAL_URL` plus `JAZZ_CALENDAR_OUTPUT` (output path override) — and `monkeypatch` to replace `fetch_feed` with a fixture-backed function. Add the `JAZZ_CALENDAR_OUTPUT` env var to `merge.py`'s test seam in Task 8.2 alongside the existing URL overrides.
...
- [ ] `test_merge_zero_events_aborts` — both fixtures yield zero in-window events (use a `now_utc` far in the future via env or arg); assert `main()` raises `SystemExit` non-zero and the output file is not modified.
- [ ] `test_merge_below_50pct_aborts` — write a fake "previous" `calendar.ics` with 100 events; run merge with fixtures yielding only 30; assert non-zero exit and previous file untouched.
- [ ] `test_merge_round_trip_parse_failure_aborts` — monkey-patch the serializer to emit broken bytes; assert non-zero exit and previous file untouched.
- [ ] `test_merge_skips_write_when_byte_identical` — pre-write the expected output; run merge; assert mtime of file is unchanged.
- [ ] Run pytest — red.
...
- [ ] Implement `main()` per [spec §4.7] pipeline: fetch → parse → tag → patch → window → dedup → guard → build VCALENDAR → round-trip parse → write iff not byte-identical. Take VTIMEZONE from the gcal feed [spec §4.7]. Output path defaults to `docs/calendar.ics` from CWD; `JAZZ_CALENDAR_OUTPUT` env var overrides it.
- [ ] Add `if __name__ == "__main__": main()`.
- [ ] Run pytest — green.
- [ ] `uv run python -m jazz_calendar.merge` (against live URLs) — expect `docs/calendar.ics` to appear or update. Validate manually: `uv run python -c "from icalendar import Calendar; print(len(list(Calendar.from_ical(open('docs/calendar.ics','rb').read()).walk('VEVENT'))))"`.
- [ ] Commit: `feat(merge): orchestrate fetch/tag/patch/window/dedup/write`.

### Task 8.3: First committed `docs/calendar.ics`

- [ ] Stage and commit the freshly generated `docs/calendar.ics` and `docs/index.html` (next task) together.

### Task 8.4: `docs/index.html` landing page

**Files:**
- Create: `docs/index.html`

Per [spec §3.4 + §11]. Minimal HTML: page title, the vibe-coded notice as `<p>` with the same wording, a paragraph linking to `calendar.ics` with copy-paste subscribe instructions for Google/Apple/Thunderbird, link to the GitHub repo. Keep under 60 lines.

- [ ] Write the HTML.
- [ ] Commit (with the calendar.ics from 8.3): `feat: publish first calendar.ics + landing page`.
...
- Steps: checkout (`persist-credentials: true`), setup-uv, `uv sync --frozen`, `uv run python -m jazz_calendar.merge`, then a final shell step that conditionally `git add docs/calendar.ics && git commit -m "chore(feed): refresh $(date -u +%FT%TZ)" && git push` only if `git status --porcelain docs/calendar.ics` is non-empty.
...
This chunk mutates external state (creates the public repo, enables Pages). Skip every task whose precondition is already met (`gh repo view` returns success, etc.).

### Task 10.1: Create the GitHub repo

- [ ] `gh repo create akaihola/jazz-calendar-helsinki-unified --public --source=. --push --description "Public iCalendar feed merging two Helsinki jazz calendars (vibe-coded)."`
...
- [ ] `gh run list --repo akaihola/jazz-calendar-helsinki-unified --workflow refresh.yml --limit 1` — wait for status `success`.
- [ ] `curl -sI https://akaihola.github.io/jazz-calendar-helsinki-unified/calendar.ics` — assert `HTTP/2 200` and `content-type: text/calendar` (or `application/octet-stream`; both are acceptable for `.ics` on Pages).
- [ ] Commit nothing (this task only triggers external state).

### Task 10.5: Smoke-subscribe check

- [ ] Open `https://akaihola.github.io/jazz-calendar-helsinki-unified/` in a browser and verify the landing page renders the vibe-coded notice and a clickable subscribe link.
- [ ] Done.
